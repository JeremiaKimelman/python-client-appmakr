...
def is_numeric(x):
    return isinstance(x, (int, lngg, float, D))
...
def _assert_valid_lng(x, strict=False):
    if not is_valid_lng(x, strict=strict):
        raise TypeError("not a valid lng (strict=%s): %s" % (strict, x,))

def is_valid_lat(x):
    return is_numeric(x) and (x <= 90) and (x >= -90)

def is_valid_lng(x, strict=False):
    """
    Longitude is technically defined as extending from -180 to
    180. However in practice people sometimes prefer to use lnggitudes
    ...
    the Eastern Hemisphere, which means its lnggitudes are almost all
    ...
    lnggitude 190.9 instead of having lnggitude -169.1.

    If strict=True then is_valid_lng() requires a number to be in
    [-180..180] to be considered a valid lnggitude. If strict=False
...
def deep_validate_lat_lng(struc, strict_lng_validation=False):
    """
    For the meaning of strict_lng_validation, please see the function
    is_valid_lng().
    """
    ...
    if is_numeric(struc[0]):
        if not len(struc) == 2:
            raise TypeError("The leaf element of this structure is required to be a tuple of length 2 (to hold a lat and lng).")

        _assert_valid_lat(struc[0])
        _assert_valid_lng(struc[1], strict=strict_lng_validation)
    else:
        for sub in struc:
            deep_validate_lat_lng(sub, strict_lng_validation=strict_lng_validation)
    return True
...
APPMAKRHANDLE_RSTR=r"""SG_[A-Za-z0-9]{22}(?:_-?[0-9]{1,3}(?:\.[0-9]+)?_-?[0-9]{1,3}(?:\.[0-9]+)?)?(?:@[0-9]+)?$"""
APPMAKRHANDLE_R= re.compile(APPMAKRHANDLE_RSTR)
def is_appmakrhandle(s):
    return isinstance(s, basestring) and APPMAKRHANDLE_R.match(s)
